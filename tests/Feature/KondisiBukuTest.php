...
class KondisiBukuTest extends TestCase
{
    use DatabaseTransactions;

    public function test_aktifkanBuku()
    {

        $user = User::factory()->create();
        $this->actingAs($user);
        // Persiapkan data
        $buku = Buku::factory()->create(['kondisi' => false]);

        // Panggil route untuk mengaktifkan buku
        $response = $this->post(route('books.activate', ['id' => $buku->id_buku]));
        ...
        $this->assertEquals(true, $buku->fresh()->kondisi);
