                ...
                <table class="table table-striped table-bordered table-condensed">
                    ...
                    <tbody>
                        @forelse($bukuDipinjam as $index => $peminjaman)
                            <tr>
                                <td>{{ $loop->iteration + $bukuDipinjam->perPage() * ($bukuDipinjam->currentPage() - 1) }}
                                </td>
                                <td>{{ $peminjaman->anggota->nama_anggota }}</td>
                                <td class="hide-on-small">{{ $peminjaman->anggota->nomor_anggota }}</td>
                                <td>{{ $peminjaman->buku->judul_buku }}</td>
                                <td>{{ $peminjaman->created_at }}</td>
                                <td>
                                    @if ($peminjaman->status == 1)
                                        <!-- Tombol Kembalikan -->
                                        <form id="kembalikanForm{{ $peminjaman->id }}"
                                            action="{{ route('admin.peminjaman.kembalikan', $peminjaman->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('PUT')
                                            <button type="button" class="btn btn-warning"
                                                onclick="kembalikanBukuAnggota({{ $peminjaman->id }})">Kembalikan</button>
                                        </form>
                                    @endif
                                </td>
                            </tr>
