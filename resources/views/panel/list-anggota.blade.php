    ...
    <script>
        function filterAnggota() {
            ...
            function shouldDisplayRow(txtValueNomor, txtValueNama, txtValueJurusan, txtValueKelas) {
                return (txtValueNomor.toUpperCase().indexOf(filter) > -1 || txtValueNama.toUpperCase().indexOf(filter) > -
                    1) &&
        ...
        // Pastikan elemen dengan ID jurusanFilter ada di dalam dokumen
        var jurusanFilterElement = document.getElementById("jurusanFilter");

        // Tambahkan event listener jika elemen ditemukan
        if (jurusanFilterElement) {
            jurusanFilterElement.addEventListener("change", function () {
    ...
    <div class="content">
        <div class="module">
            ...
            <div class="module-body">
                ...
                <input type="text" id="searchInput" onkeyup="searchAnggota()"
                    placeholder="Cari berdasarkan nomor anggota atau nama anggota" style="width: 45%;">
                ...
                <a href="{{ URL::route('admin.tambah-anggota') }}" style="margin-bottom:10px" class="btn btn-inverse">Tambah
                    Nomor Anggota</a>
                <a href="{{ URL::route('verifikasi-anggota') }}" style="margin-bottom:10px" class="btn btn-inverse">Daftar
                    Nomor Anggota</a>
                <table id="anggotaTable" class="table table-striped table-bordered table-condensed">
                    <thead>
                        <tr>
                            ...
                            <th>Nomor Anggota</th>
                            <th>Nama</th>
                            <th class="hide-on-small">Jurusan</th>
                            <th>Kelas</th>
                            <th>Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($anggotaList as $index => $anggota)
                            <tr>
                                ...
                                <td class="hide-on-small">{{ $anggota->jurusan }}</td>
                                <td>{{ $anggota->kelas }}</td>

                                <td>
                                    <div class="center-block"
                                        style="display: flex; flex-direction: column; align-items: center;">
                                        <a href="{{ route('list-anggota-detail', ['id' => $anggota->id_anggota]) }}"
                                            class="btn btn-primary btn-block" style="width: 75px;">Detail</a>
                                        <a href="{{ route('list-anggota-edit', ['id' => $anggota->id_anggota]) }}"
                                            class="btn btn-info btn-block" style="margin-bottom: 5px; width: 75px;">Edit</a>
                                        <form id="deleteAnggota{{ $anggota->id_anggota }}"
                                            action="{{ route('list-anggota-delete', $anggota->id_anggota) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="btn btn-danger"
                                                style="margin-bottom: 2px; width: 75px;"
                                                onclick="deleteAnggota('{{ $anggota->id_anggota }}')">Hapus</button>
                                        </form>
                                    </div>
