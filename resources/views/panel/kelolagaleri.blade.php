    ...
    <div class="content">
        <div class="module">
            ...
            <div class="module-body">
                <a href="{{ URL::route('galeri.create') }}" style="margin-bottom:10px" class="btn btn-inverse">Tambah
                    Galeri</a>
                    ...
                    <tbody>
                        @foreach ($galeri as $key => $gambar)
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                <td>{{ $gambar->judul }}</td>
                                <td><img src="{{ asset('storage/' . $gambar->gambar_galeri) }}" alt="{{ $gambar->judul }}"
                                        width="100"></td>
                                <td class="hide-on-small">{{ $gambar->deskripsi }}</td>
                                <td>
                                    <a href="{{ route('galeri.edit', ['id' => $gambar->id]) }}"
                                        class="btn btn-primary">Edit</a>
                                    <a href="{{ route('galeri.show', ['id' => $gambar->id]) }}"
                                        class="btn btn-info">Detail</a>

                                    <!-- Tambahkan SweetAlert untuk konfirmasi penghapusan -->
                                    <form id="deleteForm{{ $gambar->id }}"
                                        action="{{ route('galeri.destroy', ['id' => $gambar->id]) }}" method="POST"
                                        style="display: inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-danger"
                                            onclick="deleteConfirmation({{ $gambar->id }})">Hapus</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
