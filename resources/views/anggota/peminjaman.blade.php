    ...
    <script>
        function filterBooks() {
            ...
            function shouldDisplayRow(txtValueTitle, txtValueNumber, txtValuePengarang, txtValueCategory, txtValueYear) {
                return (txtValueTitle.toUpperCase().indexOf(filter) > -1 || txtValueNumber.toUpperCase().indexOf(filter) > -
                        1 || txtValuePengarang.toUpperCase().indexOf(filter) > -1) &&
            ...
            for (i = 0; i < tr.length; i++) {
                ...
                if (tdTitle || tdNumber || tdPengarang || tdCategory || tdYear) {
                    ...
                    if (shouldDisplayRow(txtValueTitle, txtValueNumber, txtValuePengarang, txtValueCategory,
                            txtValueYear)) {
                        tr[i].style.display = "";
                        found = true;
                    } else {
                ...
                <table id="bookTable" class="table table-striped table-bordered table-condensed">
                    ...
                    <tbody>
                        ...
                        @foreach ($daftarBukuTersedia as $buku)
                            @if ($buku->kondisi == 1)
                                <!-- Hanya tampilkan buku yang aktif -->
                                @php $nomorUrut++; @endphp <!-- Tingkatkan nomor urut hanya jika buku aktif -->
                                <tr>
                                    <td>{{ $nomorUrut }}</td>
                                    <td class="hide-on-small">{{ $buku->isbn }}</td>
                                    <td>{{ $buku->judul_buku }}</td>
                                    <td class="hide-on-small">{{ $buku->pengarang }}</td>
                                    <td class="hide-on-small">{{ $buku->penerbit }}</td>
                                    <td class="hide-on-small">{{ $buku->tahun_terbit }}</td>
                                    <td class="hide-on-small">{{ $buku->kategori->kategori }}</td>
                                    ...
                                    <td>
                                        <form action="{{ route('anggota.peminjaman.store') }}" method="POST">
                                            ...
                                            @if ($buku->status_buku === 'Not Available')
                                                <button type="button" class="btn btn-danger" disabled>Tidak
                                                    Tersedia</button>
                                            @else
